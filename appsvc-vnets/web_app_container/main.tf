...
resource "azurerm_container_registry_webhook" "main" {
  ...
  service_uri = "https://${azurerm_app_service.main.site_credential.0.username}:${azurerm_app_service.main.site_credential.0.password}@${local.webhook_domain}/docker/hook"
...
resource "azurerm_key_vault" "main" {
  ...
  dynamic "access_policy" {
    ...
    content {
      tenant_id = azurerm_app_service.main.identity.0.tenant_id
      object_id = coalesce(access_policy.value["object_id"], azurerm_app_service.main.identity.0.principal_id)
